...
def main():
    ...
    data, timing = data.get_batch_data(batch_no)
    
    # Load the model data
    model = LoadModel()
    
    # Create a result object for the results after model application   
    result = MPCABatchResult(batch_no)
    
    # Simulate that the data arrives iteratively. Each iteration a new measurement
    # per variable is included in the model.
    for t in range(1, data.shape[0]):
        var_values_online = data.iloc[:t,:]
        
        # Create a new object which can perform the online alignment of the
        # Dynamic Time Warping (DTW) algorithm        
        online_batch = BatchDataWarpingOnline(batch_no,var_values_online, timing, reference_batch)
        online_batch.dtw_online_all_phases()
    
        # Applies the MPCA (Multiway PCA) model on the DTW data 
        online_batch_MPCA = OnlineMPCABatch(online_batch.get_warping_up_to_current_moment(),
                                            online_batch.get_batch_no(),
                                            model.principal_components,
                                            model.mean,
                                            model.std)
        online_batch_MPCA.scale_values()
        online_batch_MPCA.calculate_online_values()
        
        # Attach new values to the result object and check if the error leaves
        # the confidence limits
        result.append(online_batch_MPCA)
        result.check_control_values(model.SPE_confidence_limits)
    
    # Plot the SPE trajectory and the corresponding confidence limits
    result.plot_SPE_and_confidence_limits(model.SPE_confidence_limits)
...
class LoadData():
    ...
    def get_batch_data(self, batch_no):
        """
        Method which returns the batch data of a given batch for the important
        process phases
...
class BatchData():
    ...
    def calc_phase_points(self):
        """
        Method which calculates the phase transition points for a batch    
        """  
    ...
    def get_phase(self, phase):
        """
        Method which returns the batch data of a specific process phase     
...
class BatchDataWarping(BatchData):
    ...
    def apply_warping(self):
        """
        Method which defines a function in R which uses the DTW (Dynamic Time
    ...
    def warp_alignment(self):
        """
        Method which returns warped query batch       
    ...
    def plot_alignment(self, var_no):
        """
        Method which plots the current alignment of the query batch     
...
class BatchDataWarpingOnline(BatchDataWarping):
    ...
    def get_current_phase_no(self):
        """
        Method which returns the number of the current batch phase        
    ...
    def apply_warping_online(self):
        """ 
        Method defines necessary DTW function for R package and executes 
    ...
    def apply_warping(self, query, reference):
        """
        Method which warps previous phases of the batch. No online alignment 
        needed, since phase is already known completely. 
    ...
    def warp_alignment_online(self, alignment, query):        
        """
        Method which returns the query warped according to the reference batch        
    ...
    def dtw_online_all_phases(self):
        """
        Method which calculates the online alignment of a batch with a 
    ...
    def plot_alignment(self, var_no):
        ...
        if var_no > self._variables_data_clean.shape[1]-1:
            ...
            ax1.plot(self._warping_up_to_current_moment.iloc[:,var_no].values,
                     color = 'orange')
            plt.title('Batch {}, var {}'.format(self._batch_no,
...
class OnlineMPCABatch():
    ...
    def scale_values(self):
        """
        Method which scales the current batch data with the mean and standard
    ...
    def calculate_online_values(self):
        """
        Method which calculates the SPE, the SPE per variable, and the score
...
class MPCABatchResult():
    ...
    def append(self, batch_online):
        """
        This method adds the current value for the Squared Prediction Error, the 
    ...
    def check_control_values(self, confidence_limits):
        """
        This method compares the current SPE value with the confidence limits
    ...
    def plot_SPE_and_confidence_limits(self, confidence_limits):
        """
        Method which plots the SPE trajectory and the corresponding confidence
        limits
        """        
